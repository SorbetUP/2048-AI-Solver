...
──────────────────────────────────────────────────────────────────
• Presets :
    --preset turbo      → depth 2 / 40 ms / beam 1 / prob 0.10
    --preset rollout    → depth 3 / moteur roll-out Numba
• CSV enrichi : bepp2_move, bepp2_val
• Affiche la progression toutes les 100 parties dans le worker BG.
• NEW  : flag --headless  → pas d’interface graphique.
"""
...
class DataLogger:
    ...
    def record(self, *, gid:str, idx:int, raw:int, score:int,
        ...
        for p in range(16):
            e = (raw >> (p*4)) & 0xF
            empties += e == 0
            cells.append(0 if e == 0 else 1 << e)
        row=[gid,idx,score,max(cells),empties,bepp2_move,
             round(bepp2_val,5)] + cells
        with self.lock, open(self.path,"a",newline="") as f:
            csv.writer(f).writerow(row)

# ─────────────────── fonctions IA « headless » ──────────────────
def _play_game(depth:int, ms:int, engine, csv_path:Optional[str]):
    """Joue UNE partie et (optionnellement) log toutes les grilles."""
...
def _bench_mp(n, depth, ms, workers, engine):
    ...
    with mp.Pool(workers) as pool:
        task=(depth, ms, engine, None) if engine is bepp_best_move \
             else (0, 0, engine, None)
        scores=pool.starmap(_play_game, [task]*n)
...
def _bg_worker_mp(n_games, depth, ms, csv_path, workers, engine):
    ...
    with mp.Pool(workers) as pool:
        while done<todo:
            chunk= min(64, todo-done) if todo!=float("inf") else 64
            pool.starmap(_play_game,
                         [(depth, ms, engine, csv_path)]*chunk)
            done += chunk
            if done % 100 == 0:
                print(f"[BG] {done:,} parties enregistrées dans {csv_path}",
                      flush=True)

# ───────────────────── Interface Pygame ─────────────────────────
class Pygame2048UI:
    def __init__(self, fps:int, speed:float, depth:int, ms:int,
                 logger_path:str, engine, auto=False):
        import pygame
        self.pg=pygame; self.engine=engine
        self.depth,self.ms=depth,ms
        self.logger=DataLogger(logger_path) if logger_path else None
        # géométrie & init
        self.T,self.M=120,20
        self.W=4*(self.T+self.M)+self.M; self.H=self.W+150
        pygame.init(); self.screen=pygame.display.set_mode((self.W,self.H))
        ...
        self.fps=fps; self.ai_delay=int(150/max(speed,.1))
        self.game=Game(); self.gid=str(uuid.uuid4())
        self.move_idx=0; self.ai_on=auto; self.show_pop=False
        self.last_ai=pygame.time.get_ticks(); self.buttons={}
        self._render()
    ...
    def _draw_board(self):
        ...
        pg.draw.rect(s,(237,224,200),r.inflate(30,20),border_radius=8); s.blit(txt,r)
        off=r.bottom+self.M
        ...
        for i in range(4):
            for j in range(4):
                ...
                rect=pg.Rect(self.M+j*(self.T+self.M),
                             off+i*(self.T+self.M),self.T,self.T)
                pg.draw.rect(s,self.colors.get(v,(60,58,50)),rect,border_radius=8)
                if v:
                    s.blit(self.F_BIG.render(str(v),True,self._col(v)),
                           self.F_BIG.render(str(v),True,self._col(v))
    ...
    def _draw_popup(self,msg):
        pg,s=self.pg,self.screen
        ov=pg.Surface((self.W,self.H),pg.SRCALPHA); ov.fill((0,0,0,180)); s.blit(ov,(0,0))
        ...
        for k,lbl in (("restart","Recommencer"),("quit","Quitter")):
            s.blit(self.F_BTN.render(lbl,True,(255,255,255)),
                   self.F_BTN.render(lbl,True,(255,255,255))
                   .get_rect(center=self.buttons[k].center))
    ...
    # ---------- logique ----------
    def _reset(self):
        self.game=Game(); self.gid=str(uuid.uuid4()); self.move_idx=0
        self.show_pop=False; self.last_ai=self.pg.time.get_ticks(); self._render()
    ...
    def tick(self):
        ...
        for e in self.pg.event.get():
            ...
            if e.type==self.pg.KEYDOWN and not self.show_pop:
                ...
                if e.key in km: self.ai_on=False; self._manual(km[e.key])
                if e.key==self.pg.K_a: self.ai_on=not self.ai_on; self._render()
                if e.key==self.pg.K_r: self._reset()
            if e.type==self.pg.MOUSEBUTTONDOWN and self.show_pop:
                p=self.pg.mouse.get_pos()
                if self.buttons["restart"].collidepoint(p): self._reset()
                elif self.buttons["quit"].collidepoint(p): self.pg.quit(); sys.exit()
...
if __name__ == "__main__":
    ...
    pa.add_argument("--bg")
    pa.add_argument("--bench", type=int)
    ...
    Pygame2048UI(fps=args.fps, speed=args.speed,
                 ...
                 engine=engine, auto=args.auto).run()
